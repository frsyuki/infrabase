...
class Server::Resource
	...
	attr_reader :self_addr, :qs_addrs
end
...
class Server::ModRPC
	...
	def push_route(qsaddr, rsrc)
		...
		rsrc = Route::RoutingSource.from_msgpack(rsrc)
...
class Server::ModRoute
	def initialize
		@route = Route::RoutingManager.new
		@route.set_qs_addrs($rs.qs_addrs)
		# heartbeatでtagを送っている
	...
	def do_get_routing_source
		$rs.qs_addrs.each {|qsaddr|
			...
			s.callback(:get_routing_source, @route.tag_of_qs(qsaddr)) do |err, res|
				...
				if res
					rsrc = Route::RoutingSource.from_msgpack(res)
